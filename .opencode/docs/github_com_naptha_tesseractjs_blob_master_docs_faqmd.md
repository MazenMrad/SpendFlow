# tesseract.js/docs/faq.md at master · naptha/tesseract.js · GitHub

> Source: https://github.com/naptha/tesseract.js/blob/master/docs/faq.md
> Cached: 2026-04-07T21:08:30.850Z
...
[](#why-am-i-getting-a-cannot-find-module-error-when-running-in-my-projectframework)
...
- [muPDF](https://github.com/ArtifexSoftware/mupdf) (AGPL-3.0 license)
...
Confirm parameters are identical.

Manually set `oem` and `psm` in both to account for different defaults.

Tesseract.js and the Tesseract CLI use different default `oem` and `psm` settings.

- Tesseract.js uses a default `oem` of `1` (`lstm` model only), while the Tesseract CLI uses a default `oem` of `2` (`lstm` with `legacy` fallback).

- Tesseract.js and the Tesseract API use a default `pms` of `6` (`PSM_SINGLE_BLOCK`), while the Tesseract CLI uses a default `psm` of `3` (`PSM_AUTO`).

- Confirm that all user-set parameters are identical.

Confirm language data is identical.

By default, when run with `oem` value `0` or `2`, Tesseract.js uses [these](https://github.com/naptha/tessdata/tree/gh-pages/4.0.0) language files.

- These were taken from the [tessdata](https://github.com/tesseract-ocr/tessdata) repo in the main Tesseract project.

By default, when run with `oem` value `1`, Tesseract.js uses [these](https://github.com/naptha/tessdata/tree/gh-pages/4.0.0_best_int) language files.

These were created by integerizing the language files from the [tessdata_best](https://github.com/tesseract-ocr/tessdata_best) repo in the main Tesseract project.

- This should be equivalent to using the LSTM language files from the [tessdata](https://github.com/tesseract-ocr/tessdata) which are created by combining an integerized version of `tessdata_best` with data for the Legacy model.

Confirm version is identical.

- Using a different version of Tesseract may result in different recognition results.

The exact version of Tesseract used for Tesseract.js can be found by clicking on the `tesseract` submodule in this directory:

- [https://github.com/naptha/tesseract.js-core/tree/master/third_party](https://github.com/naptha/tesseract.js-core/tree/master/third_party)
...
[](#how-does-tesseractjs-download-and-keep-traineddata)
